...
export BASE_DIR=$PWD; unzip [MYZIP]; 
tar -xf [DATA]; 
DATA_FOLDER=$(basename [DATA] .tar); 
...
for DAY_PATH in $DAYS_PATH; 
	do readarray -d '' STEAM_DIRS_PATH < <(find \"$DAY_PATH\" -maxdepth 1 -type d -name \"*STEAM*\" -print0); 
	for STEAM_DIR_PATH in \"${STEAM_DIRS_PATH[@]}\"; 
		do old_path=\"$STEAM_DIR_PATH\"; 
		STEAM_DIR_PATH=\"${STEAM_DIR_PATH// /}\"; 
		mv \"$old_path\" \"$STEAM_DIR_PATH\"; 
		for file in \"${files[@]}\"; 
			do if [[ ! -f \"$STEAM_DIR_PATH/Raw/$file\" ]]; 
				then echo \"File not found: $file in $STEAM_DIR_PATH/Raw\" && exit 1; 
			fi; 
		done; 
...
find \"$DATA_FOLDER\" -type f -name '[OUTNAME]*' -print0 | tar --null --files-from - -czf [OUT_FILE]
